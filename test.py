...
def run_SVM(name, train=True):
	if train:
		...
		elapsed = 0.0
	q = subprocess.check_output("python compute_accuracy.py dota.2.label SVM.result",shell=True)
	return (q, elapsed)

def print_test():
	q = run_test("dota.25","model_file_perceptron", "perceptron.result", "perceptron", train=True)
